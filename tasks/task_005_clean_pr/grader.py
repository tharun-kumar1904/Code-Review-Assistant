"""Grader for task_005_clean_pr — Clean PR with no bugs, should approve."""
...
def _score_action(action, gold=None):
    ...
    # For clean PRs, penalize false positives
    bug_score = 0.40  # Full marks if no bugs were supposed to be found
    issue_words = ["bug", "error", "vulnerability", "critical", "high"]
    false_alarms = sum(1 for w in issue_words if w in action_str)
    if false_alarms > 2:
        bug_score = 0.10

    action_score = 0.19  # Clean PRs get base actionability
